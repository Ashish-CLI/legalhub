...
interface FormFields {
  ...
  professionalDocument: File | null;
}
...
export default function RegisterPage() {
  ...
  async function handleSendOtp(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setErrors({ email: data.error || "Failed" });
        return;
      }
      setMessage("OTP sent!");
      setStep("otp");
  ...
  async function handleVerifyOtp(e: FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await secureJsonPost("/api/auth/verify-otp", {
  ...
  async function handleRegister(e: FormEvent) {
    ...
    if (!formData.fullName || formData.fullName.length < 2)
      ne.fullName = "Min 2 characters";
    if (!formData.phoneNumber) ne.phoneNumber = "Required";
    else {
      let p = formData.phoneNumber.trim();
      if (/^[6-9]\d{9}$/.test(p)) p = `+91${p}`;
      if (!/^\+91[6-9]\d{9}$/.test(p)) ne.phoneNumber = "Invalid number";
    }
    if (!formData.address || formData.address.length < 10)
      ne.address = "Min 10 characters";
    if (!formData.password || formData.password.length < 8)
      ne.password = "Min 8 characters";
    if (formData.password !== formData.confirmPassword)
      ne.confirmPassword = "Mismatch";
    if (!formData.idDocument) ne.idDocument = "Required";
    if (
      ["lawyer", "judge", "admin"].includes(formData.role) &&
      !formData.professionalDocument
    )
      ne.professionalDocument = "Required";
    ...
    try {
      ...
      if (formData.professionalDocument)
        body.append("professionalDocument", formData.professionalDocument);
  ...
  async function handleResend() {
    ...
    setLoading(true);
    try {
      const res = await secureJsonPost("/api/auth/send-otp", {
        email: formData.email,
      });
      const data = await res.json();
      if (!res.ok) {
        setErrors({ otp: data.error || "Failed" });
        return;
      }
  ...
  const inp = (f: string) =>
    `w-full rounded-xl border bg-white/[0.04] py-3 px-4 text-white placeholder-slate-500 outline-none transition-all focus:bg-white/[0.07] focus:ring-2 ${errors[f] ? "border-red-500/40 focus:ring-red-500/20" : "border-white/[0.08] focus:border-blue-500/40 focus:ring-blue-500/20"}`;
  const Spin = () => (
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#0a0e1a] px-4">
        <motion.div
          ...
          className="w-full max-w-md rounded-3xl border border-white/[0.08] bg-white/[0.04] p-8 text-center backdrop-blur-xl sm:p-10"
        >
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
          ...
          <Link
            href="/login"
            className="inline-block w-full rounded-xl bg-gradient-to-r from-blue-600 to-blue-500 py-3 text-center font-semibold text-white shadow-lg shadow-blue-600/20"
          >
            Go to Login
          </Link>
        </motion.div>
    ...
    <div className="flex min-h-screen">
      ...
      <div className="relative flex w-full flex-col items-center justify-center overflow-hidden bg-[#0a0e1a] px-4 py-6 lg:w-1/2">
        ...
        <motion.div
          ...
          className="relative z-10 w-full max-w-md overflow-y-auto px-2"
        >
          {/* Logo */}
          <div className="mb-6">
            ...
            {stepsMeta.map((s, i) => (
              <div key={s.key} className="flex flex-1 items-center">
                <div className="flex flex-col items-center gap-1">
                  <div
                    className={`flex h-9 w-9 items-center justify-center rounded-full border transition-all ${i === si ? "border-blue-500/50 bg-blue-500/20 text-blue-400 shadow-lg shadow-blue-500/20" : i < si ? "border-emerald-500/50 bg-emerald-500/20 text-emerald-400" : "border-white/[0.08] bg-white/[0.03] text-slate-500"}`}
                  >
                    {i < si ? (
                      <svg
                        className="h-4 w-4"
                  ...
                  <span
                    className={`text-[10px] font-medium uppercase tracking-wider ${i <= si ? "text-slate-300" : "text-slate-600"}`}
                  >
                    {s.label}
                  </span>
                </div>
                {i < 2 && (
                  <div className="mx-2 mb-5 h-px flex-1">
                    <div
                      className={`h-full transition-all ${i < si ? "bg-emerald-500/50" : "bg-white/[0.06]"}`}
                    />
                  </div>
                )}
              </div>
          ...
          <AnimatePresence mode="wait">
            {message && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0 }}
                className={`mb-4 rounded-xl border px-4 py-3 text-sm ${message.includes("verified") || message.includes("sent") || message.includes("resent") || message.includes("Successful") ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-400" : "border-red-500/20 bg-red-500/10 text-red-400"}`}
              >
                {message}
              </motion.div>
            )}
                ...
                <div>
                  ...
                  <div className="group relative">
                    <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4">
                      <svg
                        className="h-5 w-5 text-slate-500 transition-colors group-focus-within:text-blue-400"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        strokeWidth={1.5}
              ...
              >
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <button
                    ...
                    className="font-semibold text-blue-400 hover:text-blue-300 disabled:text-slate-600"
                  >
                    Resend
                  </button>
                </div>
            ...
            {step === "details" && (
              ...
              >
                ...
                <div>
                  ...
                  <div className="flex">
                    <span className="inline-flex items-center rounded-l-xl border border-r-0 border-white/[0.08] bg-white/[0.06] px-3 text-sm text-slate-400">
                      +91
                    </span>
                    <input
                      id="phoneNumber"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className={`resize-none ${inp("address")}`}
                  />
                  <Err field="address" />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    value={formData.role}
                    onChange={handleChange}
                    className="w-full rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-3 text-white outline-none focus:border-blue-500/40 focus:ring-2 focus:ring-blue-500/20"
                  >
                    <option value="client" className="bg-[#0a0e1a]">
                      Client
                    </option>
                    <option value="lawyer" className="bg-[#0a0e1a]">
                      Lawyer
                    </option>
                    <option value="judge" className="bg-[#0a0e1a]">
                      Judge
                    </option>
                    <option value="admin" className="bg-[#0a0e1a]">
                      Admin
                    </option>
                  </select>
                </div>
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-300">
                    ID Document{" "}
                    <span className="text-slate-500">(PDF/JPG, 5MB)</span>
                  </label>
                  <div
                    onClick={() => idDocRef.current?.click()}
                    className={`flex cursor-pointer items-center gap-3 rounded-xl border-2 border-dashed px-4 py-3 transition-all hover:border-blue-500/40 hover:bg-blue-500/5 ${errors.idDocument ? "border-red-500/30" : "border-white/[0.08]"}`}
                  >
                ...
                {needsProf && (
                  <div>
                    ...
                    <div
                      ...
                      className={`flex cursor-pointer items-center gap-3 rounded-xl border-2 border-dashed px-4 py-3 transition-all hover:border-amber-500/40 hover:bg-amber-500/5 ${errors.professionalDocument ? "border-red-500/30" : "border-white/[0.08]"}`}
                    >
                      ...
                      <span className="truncate text-sm text-slate-400">
                        {formData.professionalDocument
                          ? formData.professionalDocument.name
                          : "Upload professional doc"}
                      </span>
                    </div>
                    <input
                      ref={profDocRef}
                      ...
                      onChange={handleFile}
                      className="hidden"
                      aria-label="Upload professional document"
                    />
                    <Err field="professionalDocument" />
                  </div>
                )}
                <Btn>
                  {loading ? (
        ...
        <div className="relative z-10 flex h-full flex-col items-center justify-center px-12 text-center">
          ...
          >
            ...
            <p className="mx-auto max-w-sm text-base text-blue-100/70">
              Thousands of legal professionals trust LegalHub for case
